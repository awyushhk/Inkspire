...
export default function Hero() {
  ...
  const rotateRight = useTransform(scrollYProgress, [0, 1], [0, 12]);
  const xLeft = useTransform(scrollYProgress, [0, 1], [0, -60]);
  const xRight = useTransform(scrollYProgress, [0, 1], [0, 60]);
  // Start much lower (250px) and rise into view
  const yOffset = useTransform(scrollYProgress, [0, 1], [250, 0]);

  return (
    <section
      ref={containerRef}
      className="relative h-[250vh] bg-background w-full z-10"
    >
